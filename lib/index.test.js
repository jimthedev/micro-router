...
test('nested async handlers', async t => {
  const hello = async req =>
    await Promise.resolve(`Hello ${req.params.msg} ${req.params.file} ${req.query.time}`)

  const routes = router(
    get('/hello/:msg/file/:file', hello)
  )

  const url = await server(routes)
  const response = await request(`${url}/hello/world/file/readme.md?time=now`)

  t.is(response, 'Hello world readme.md now')
})
